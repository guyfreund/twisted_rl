...
from mujoco_infra.mujoco_utils.mujoco import convert_topology_to_str, load_pickle
...
def bfs_new(start, goal, max_depth=None, with_h=True, h_path="system_flow/metrics/h_values.txt", one_step=False):
    # using max_depth to limit search. mainly used for data generation.
...
def bfs_all_path_new(start, goal, max_depth, h_path, with_h=True, only_R1=False):
    # find all paths connecting start to goal.
    # limit search using max_depth.
    if with_h:
        h_scores = load_pickle(h_path)
