from mujoco_infra.mujoco_utils.topology.representation import AbstractState, Face
import numpy as np
...
def intersection(first_line_start_point, first_line_end_point, second_line_start_point, second_line_end_point):
    ...
    if intersection_lines.is_empty:
        return None
...
def find_new_intersections(state):
...
def state2topology(state, update_edges=True, update_faces=True, full_topology_representation=False):
    new_intersections = find_new_intersections(state)
