...
class PoolWorker(mp_context.Process):
    ...
    def run(self) -> None:
        ...
        while True:
            self.collect_gc()
            try:
                query_data = self.requests_queue.get(block=True, timeout=0.000001)
                response = self.do_work(query_data)
                self.response_queue.put(response)
            except queue.Empty:
                pass
            try:
                termination_message = self.worker_specific_request_queue.get(block=True, timeout=0.000001)
...
class WorkerPool:
    ...
    def _get_from_response_queue(self):
        ...
        while True:
            start_sleep_time = time.time()
            try:
                message = self.response_queue.get(block=True, timeout=0.000001)
